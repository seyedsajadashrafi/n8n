"""
A minimal-from-scratch implementation of ResNet (BasicBlock and Bottleneck)
without using torchvision.models.

Provides constructors: resnet18, resnet34, resnet50, resnet101, resnet152.

API is intentionally small and similar to torchvision's implementation so it is
easy to use in tests or training scripts.
"""
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes: int, planes: int, stride: int = 1, downsample: Optional[nn.Module] = None,
        ...
        # Both convs have stride 1 except first conv may have stride > 1
...
class Bottleneck(nn.Module):
    # Bottleneck in torchvision has expansion 4
    expansion: int = 4
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block: Type[nn.Module], planes: int, blocks: int, stride: int = 1, dilate: bool = False) -> nn.Sequential:
    ...
    def _forward_impl(self, x: torch.Tensor) -> torch.Tensor:
        # See note: torchvision's ResNet uses _forward_impl for TorchScript friendliness
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
...
def _resnet(arch: str, block: Type[nn.Module], layers: List[int], **kwargs: object) -> ResNet:
    model = ResNet(block, layers, **kwargs)
    return model


def resnet18(**kwargs: object) -> ResNet:
    return _resnet('resnet18', BasicBlock, [2, 2, 2, 2], **kwargs)


def resnet34(**kwargs: object) -> ResNet:
    return _resnet('resnet34', BasicBlock, [3, 4, 6, 3], **kwargs)


def resnet50(**kwargs: object) -> ResNet:
    return _resnet('resnet50', Bottleneck, [3, 4, 6, 3], **kwargs)


def resnet101(**kwargs: object) -> ResNet:
    return _resnet('resnet101', Bottleneck, [3, 4, 23, 3], **kwargs)


def resnet152(**kwargs: object) -> ResNet:
